...
def val_fn(val_loader, model, loss_fn, epoch=None):
    ...
    with torch.no_grad():
        for data, target in val_loader: 
            ...
            inter_over_union = iou(target, torch.where(output >= THRESHOLD, 1, 0))
...
def train_one_go(train_dataloader, model, optimizer, scheduler, loss_fn, n_epochs, check=False, best_val=None):
    ...
    for epoch in epochs:
        train_metrics = train_fn(train_loader=train_dataloader, model=model, optimizer=optimizer, scheduler=scheduler, loss_fn=loss_fn, epoch=epoch, num_epochs=n_epochs)
        val_metrics = val_fn(val_loader=val_expert_dataloader, model=model, loss_fn=loss_fn, epoch=epoch)
...
def train_with_history(train_dataloader, model, optimizer, scheduler, loss_fn, n_epochs, filename, check=False, best_val=None):
    history, epochs, best_val, filepath = train_one_go(train_dataloader ,model, optimizer, scheduler, loss_fn, n_epochs, check, best_val)
    plot_training_history(history, list(epochs), filename=filename)
    return filepath, best_val

print(' --- Loading Data ---')
dict = load_data_smart('train.pkl', device=DEVICE)
print(' --- End Load ---')
...
expert_imgs, expert_filtered, expert_segs = resize_segmentation_data_simple(expert_imgs, expert_filtered, expert_segs, IMAGE_HEIGHT, IMAGE_WIDTH)
...
train_expert_imgs, val_expert_imgs, train_expert_segs, val_expert_segs = train_test_split(expert_imgs, expert_segs, test_size=0.2, random_state=42)

convert_to_tensor = lambda x: torch.from_numpy(x).to(torch.float32).to(DEVICE)
train_amateur_imgs, train_amateur_segs = convert_to_tensor(train_amateur_imgs), convert_to_tensor(train_amateur_segs)
train_expert_imgs, train_expert_segs = convert_to_tensor(train_expert_imgs), convert_to_tensor(train_expert_segs)
val_expert_imgs, val_expert_segs = convert_to_tensor(val_expert_imgs), convert_to_tensor(val_expert_segs)
expert_imgs, expert_segs = convert_to_tensor(expert_imgs), convert_to_tensor(expert_segs)

train_amateur_dataset = CustomDataset(train_amateur_imgs, train_amateur_segs, channels=3, device=DEVICE, apply_transform=True)
train_expert_dataset = CustomDataset(train_expert_imgs, train_expert_segs, channels=3, device=DEVICE, apply_transform=True)
last_expert_dataset = CustomDataset(expert_imgs, expert_segs, channels=3, device=DEVICE, apply_transform=False)
val_expert_dataset = CustomDataset(val_expert_imgs, val_expert_segs, channels=3, device=DEVICE, apply_transform=False)

train_amateur_dataloader = DataLoader(dataset=train_amateur_dataset, batch_size=BATCH_SIZE)
train_expert_dataloader = DataLoader(dataset=train_expert_dataset, batch_size=BATCH_SIZE)
last_expert_dataloader = DataLoader(dataset=last_expert_dataset, batch_size=BATCH_SIZE)
val_expert_dataloader = DataLoader(dataset=val_expert_dataset, batch_size=BATCH_SIZE)
...
train_with_history(train_amateur_dataloader, model, optimizer, scheduler, loss_fn, NUM_EPOCHS_AMATEUR, 'amateur_history.png')

# # them train on experts
filepath, best_val = train_with_history(train_expert_dataloader, model, optimizer, scheduler, loss_fn, NUM_EPOCHS_EXPERT, 'expert_history.png', check=True, best_val=0.0)

# lastly train on whole expert dataset
filepath, _ = train_with_history(last_expert_dataloader, model, optimizer, scheduler, loss_fn, NUM_EPOCHS_LAST, 'last_history.png', check=True, best_val=best_val)
