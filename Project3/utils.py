    ...
    with tqdm(data) as loop:
        for item in loop:
            if filter(filter_expert, filter_amateur, item):
                ...
                box_imgs = imgs[top_left[0]:bottom_right[0],
                                top_left[1]:bottom_right[1], :]
                box_segs = segs[top_left[0]:bottom_right[0],
                                top_left[1]:bottom_right[1], :]
...
def load_data_smart(file_path, device='cpu'):
    ...
    with tqdm(data) as loop:
        for item in loop:
            ...
            W, H, S = robust_nmf_breg(
                video, n_comp=2, iterations=250, λ=0.02, tol=1e-6, device=device)
    ...
    with tqdm(data) as loop:
        for item in loop:
            if filter(filter_expert, filter_amateur, item):
                ...
                video = item['video']
                if sparse:
                    _, _, S = robust_nmf_breg(
                        video, n_comp=2, iterations=150, λ=0.01)
                    video = S
...
def resize_segmentation_data(imgs, boxes, target_height, target_width):
    train_imgs, train_segs = np.zeros((len(imgs), target_height, target_width)), np.zeros(
        (len(boxes), target_height, target_width))
    for i, (img, seg) in enumerate(zip(imgs, boxes)):
        img = cv2.resize(img, (target_height, target_width),
                         interpolation=cv2.INTER_LINEAR_EXACT)
        seg = Image.fromarray(seg).resize(
            (target_height, target_width), resample=Image.BILINEAR)
...
def resize_segmentation_data_simple(imgs, segs, target_height, target_width):
    train_imgs, train_segs = np.zeros((len(imgs), target_height, target_width)), np.zeros(
        (len(segs), target_height, target_width))
    for i, (img, seg) in enumerate(zip(imgs, segs)):
        ...
        if (seg.shape[-2] != target_height or seg.shape[-1] != target_width):
            seg = np.asarray(Image.fromarray(seg).resize(
                (target_height, target_width), resample=Image.BILINEAR))
...
def resize_segmentation_data_simple(imgs, filtered, segs, target_height, target_width):
    ...
    for i, (img, filt, seg) in enumerate(zip(imgs, filtered, segs)):
        ...
        if (filt.shape[-2] != target_height or filt.shape[-1] != target_width):
            filt = cv2.resize(filt, (target_height, target_width),
                              interpolation=cv2.INTER_LINEAR_EXACT)
        if (seg.shape[-2] != target_height or seg.shape[-1] != target_width):
            seg = np.asarray(Image.fromarray(seg).resize(
                (target_height, target_width), resample=Image.BILINEAR))
...
def preprocess_segmentation_data(imgs, rest):
    ...
    for i, (img, rest_img) in enumerate(zip(imgs, rest)):
        ...
        augmented_img = np.concatenate(
            [ndarray[None, ...] for ndarray in ndarrays], axis=0)
...
def overlay_segmentation_countor_grayscale_for_gif(image, mask, bounding_box=None, color='red', colormap=None):
    fig, ax = overlay_segmentation_countor_grayscale(
        image, mask, bounding_box=bounding_box, color=color, colormap=colormap)
...
def plot_training_history(history: dict, epochs, title="Training and Validation Metrics", filename='plot_metrics.png'):
    ...
    assert all([any([rkey == key for rkey in required_keys])
