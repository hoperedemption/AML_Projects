...
def process_ecg(ecg_signal, freq, i):
    try:
        ...
        hrv_specific = pyhrv.hrv(interpolated_nn_intervals, rpeaks=rpeaks, signal=filtered_scaled_data, sampling_rate=freq)
        
        # filter out the features
        dict_hrv_specific = dict(hrv_specific)
...
def process_all_ecg(signals, freq=SAMPLE_FREQ, n_jobs=-1):    
    results = Parallel(n_jobs=n_jobs, backend='multiprocessing')(delayed(process_ecg)(signals.iloc[i, :], freq, i) for i in range(signals.shape[0]))
...
# sample_signal = X.iloc[1368, :]
# features_cct = process_ecg(sample_signal, SAMPLE_FREQ)
# print(features_cct.shape)
# features_cct.to_csv('__F__.csv')
